...
struct URLRoutingTranslatingPerformanceTests {
    ...
    struct ParsingPerformanceTests {
        ...
        func stringLiteralParsingPerformance() throws {
            ...
            print("   Average time per operation: \(String(format: "%.2f", timeInSeconds / Double(iterations) * 1_000_000))μs")
        ...
        func performanceComparisonStringVsTranslatedString() throws {
            ...
            let stringTimeInSeconds = Double(stringResult.components.seconds) + Double(stringResult.components.attoseconds) / 1e18
            let translatedTimeInSeconds = Double(translatedStringResult.components.seconds) + Double(translatedStringResult.components.attoseconds) / 1e18
            ...
            print("   String Ops/sec: \(String(format: "%.0f", Double(iterations) / stringTimeInSeconds))")
            print("   TranslatedString Time: \(translatedStringResult)")
            print("   TranslatedString Time (seconds): \(translatedTimeInSeconds)")
            print("   TranslatedString Ops/sec: \(String(format: "%.0f", Double(iterations) / translatedTimeInSeconds))")
            print("   Performance ratio: \(String(format: "%.2f", performanceRatio))x")
            print("   TranslatedString is \(String(format: "%.2f", performanceRatio))x slower than String")
            print("   Overhead per operation: \(String(format: "%.2f", (translatedTimeInSeconds - stringTimeInSeconds) / Double(iterations) * 1_000_000))μs")

            #expect(performanceRatio < 10.0, "TranslatedString should not be more than 10x slower than String")
        }
    }
    ...
    struct MultiLanguagePerformanceTests {
        ...
        func performanceWithTwoLanguages() throws {
            ...
            print("   Time: \(result)")
            print("   Time (seconds): \(timeInSeconds)")
            print("   Operations per second: \(String(format: "%.0f", Double(iterations) / timeInSeconds))")
        ...
        func languageCountScalingAnalysis() throws {
            ...
            for (languageCount, duration) in results {
                let timeInSeconds = Double(duration.components.seconds) + Double(duration.components.attoseconds) / 1e18
                let opsPerSecond = Double(iterations) / timeInSeconds

                print("   \(String(format: "%2d", languageCount)) languages: \(duration) (\(String(format: "%.0f", opsPerSecond)) ops/sec)")
            }

            // Performance should be consistent since we're always matching the first language
            let firstResult = Double(results[0].1.components.seconds) + Double(results[0].1.components.attoseconds) / 1e18
            let lastResult = Double(results.last!.1.components.seconds) + Double(results.last!.1.components.attoseconds) / 1e18
            ...
            #expect(performanceDegradation < 2.0, "Performance should not degrade significantly with more languages when matching current language")
        }
    }
    ...
    struct URLGenerationPerformanceTests {
        ...
        func stringURLGenerationPerformance() throws {
            ...
            print("   Operations per second: \(String(format: "%.0f", operationsPerSecond))")
            print("   Average time per URL: \(String(format: "%.2f", timeInSeconds / Double(iterations * 3) * 1_000_000))μs")
        ...
        func urlGenerationDifferentLanguagesPerformance() throws {
            ...
            let englishTimeInSeconds = Double(englishResult.components.seconds) + Double(englishResult.components.attoseconds) / 1e18
            let dutchTimeInSeconds = Double(dutchResult.components.seconds) + Double(dutchResult.components.attoseconds) / 1e18

            print("📊 URL GENERATION - Language Comparison:")
            print("   English Time: \(englishResult)")
            print("   English Ops/sec: \(String(format: "%.0f", Double(iterations * 3) / englishTimeInSeconds))")
            print("   Dutch Time: \(dutchResult)")
            print("   Dutch Ops/sec: \(String(format: "%.0f", Double(iterations * 3) / dutchTimeInSeconds))")
            print("   Performance difference: \(String(format: "%.2f", dutchTimeInSeconds / englishTimeInSeconds))x")
            print("   ✅ Should be similar performance regardless of language")

            let performanceDifference = abs(dutchTimeInSeconds - englishTimeInSeconds) / min(dutchTimeInSeconds, englishTimeInSeconds)
            #expect(performanceDifference < 0.5, "Performance should be similar regardless of current language")
        }
    }
    ...
    struct RouterIntegrationPerformanceTests {
        ...
        func roundTripPerformance() throws {
            let iterations = 5_000
            let router = TestRouter()
            let routes: [TestRoute] = [.home, .about, .contact, .privacyPolicy, .generalTerms, .newsletter]
            let clock = ContinuousClock()
            ...
            print("   Operations per second: \(String(format: "%.0f", operationsPerSecond))")
            print("   Average round-trip time: \(String(format: "%.2f", timeInSeconds / Double(iterations * routes.count) * 1000))ms")
        ...
        func errorHandlingPerformance() throws {
            let iterations = 5_000
            let router = TestRouter()
            let invalidPaths = ["/invalid", "/nonexistent", "/fake-path", "/unknown-url", "/missing-route"]
            let clock = ContinuousClock()
            ...
            print("   Operations per second: \(String(format: "%.0f", operationsPerSecond))")
            print("   Average error handling time: \(String(format: "%.2f", timeInSeconds / Double(totalOperations) * 1_000_000))μs")
    ...
    struct MemoryUsageTests {
        ...
        func memoryUsageComparison() throws {
            ...
            struct StringRouter: ParserPrinter {
                var body: some URLRouting.Router<TestRoute> {
                    OneOf {
                        ...
                        URLRouting.Route(.case(TestRoute.generalTerms)) { Path { "general-terms-and-conditions" } }
            ...
            print("   Memory overhead: \(translatedRouterMemoryIncrease - stringRouterMemoryIncrease) bytes")
